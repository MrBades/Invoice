...
def parse_smart_input(text):
    ...
    if api_key and online:
        try:
            ...
            # Backward compatibility fix: extract first item name to prevent views.py KeyError
            first_item_name = "General Goods"
            if items and len(items) > 0:
                first_item_name = items[0].get('product_name', 'General Goods')
            elif data.get('product_name'):
                first_item_name = data.get('product_name')

            return {
                'intent': 'invoice',
                'product_name': first_item_name,  # Essential mapping field for views.py line 120
